...
function beep(freq, type, dur, vol) {
  try {
    ...
    g.gain.exponentialRampToValueAndAtTime?.(0.001, ctx.currentTime+(dur||0.1));
...
const sfxSelect = () => { beep(520,"sine",0.1,0.07); setTimeout(()=>beep(780,"sine",0.15,0.05),90); };
...
function Corners({color,size,t}){
  const c=color||BL,sz=size||20,th=t||2;
...
function Glitch({children,style={}}){
  return (
    <span style={{position:"relative",display:"inline-block",...style}}>
      ...
      `}</style>
      <span data-text={children} style={{position:"relative"}}>{children}</span>
...
function CRMNetwork3D(){
  ...
  useEffect(()=>{
    ...
    const edges=[
      [0,3],[1,3],[2,3],   // POS/EC/決済 → POINTQUIC
      [3,4],[3,5],         // POINTQUIC → LINE/CRM
      [4,6],[5,7],[3,8],   // outputs to customers
    ];
    ...
    const flowState=Array.from({length:particleCount},(_,i)=>({
      edge:i%edges.length, t:Math.random(),
    }));
    ...
    const animate=()=>{
      ...
      nodeMeshes.forEach((m,i)=>{
        const s=1+0.08*Math.sin(elapsed*2+i*1.2);
        m.scale.setScalar(s);
    ...
    window.addEventListener("mousemove",e=>{
      ...
      scene.rotation.y+=dx*0.006; lastMX=e.clientX;
    });
    let lastTX=null;
    renderer.domElement.addEventListener("touchmove",e=>{
      if(lastTX===null){lastTX=e.touches[0].clientX;return;}
      scene.rotation.y+=(e.touches[0].clientX-lastTX)*0.008;
  ...
  return (
    <div style={{position:"relative",width:"100%"}}>
      ...
      <div style={{position:"absolute",bottom:8,right:10,fontFamily:C,fontSize:"clamp(.5rem,1.8vw,.6rem)",
        color:"rgba(14,165,233,.35)"}}>ドラッグで回転</div>
      <div style={{position:"absolute",top:8,left:10,fontFamily:C,fontSize:"clamp(.5rem,1.8vw,.6rem)",
        color:"rgba(14,165,233,.5)",lineHeight:1.8}}>
  ...
  return (
    <Shell>
      <style>{`
        ...
        @keyframes pulseglow{0%,100%{box-shadow:0 0 20px rgba(14,165,233,.3)}50%{box-shadow:0 0 60px rgba(14,165,233,.7)}}
      `}</style>
      <div style={{textAlign:"center",display:"flex",flexDirection:"column",alignItems:"center",
        gap:"clamp(1.2rem,4vw,2rem)"}}>

        <div style={{opacity:ph>=1?1:0,transition:"opacity .8s",fontFamily:C,
          color:`rgba(14,165,233,.5)`,fontSize:"clamp(.7rem,2.5vw,.85rem)",letterSpacing:".3em"}}>
          POINTQUIC / 株式会社キーラボ
        </div>

        <div style={{opacity:ph>=2?1:0,transition:"opacity 1s",
          transform:ph>=2?"translateY(0)":"translateY(24px)"}}>
          <div style={{
            fontFamily:F,fontWeight:900,
            fontSize:"clamp(1.8rem,7vw,4.2rem)",
            lineHeight:1.1,letterSpacing:"-.02em",color:"#f1f5f9",
          }}>
            <span style={{display:"block",color:"rgba(241,245,249,.4)",
              fontSize:"clamp(.8rem,2.5vw,.95rem)",fontWeight:400,fontFamily:C,
              letterSpacing:".2em",marginBottom:".5rem"}}>DATA-DRIVEN CRM</span>
            取引データで、<br/>
        ...
        <div style={{opacity:ph>=3?1:0,transition:"opacity .8s",maxWidth:420,
          fontFamily:F,fontSize:"clamp(.85rem,3vw,1rem)",color:"#64748b",lineHeight:1.8}}>
          今日は、<span style={{color:AM,fontWeight:700}}>3つの確認</span>から始めます。
        </div>

        {ph>=3&&(
          <div style={{fontFamily:C,color:"rgba(100,116,139,.35)",
            fontSize:"clamp(.65rem,2.5vw,.78rem)",display:"flex",alignItems:"center",gap:".6rem"}}>
...
function S1_Purpose(){
  ...
  return (
    <Shell>
      ...
      <h2 style={{fontFamily:F,fontSize:"clamp(1.4rem,5vw,2.2rem)",fontWeight:800,color:"#f1f5f9",
        lineHeight:1.2,margin:"0 0 clamp(1rem,4vw,1.8rem)"}}>
...
function S2_Change(){
  ...
  return (
    <Shell>
      ...
      <h2 style={{fontFamily:F,fontSize:"clamp(1.3rem,4.5vw,2rem)",fontWeight:800,color:"#f1f5f9",
        lineHeight:1.3,margin:"0 0 clamp(.8rem,3vw,1.5rem)"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12,
        ...
        {[
          ...
          <div key={ci} style={{padding:"clamp(.8rem,3vw,1rem)",border:`1px solid ${col.color}30`,
            background:`${col.color}08`,borderRadius:4,borderTop:`2px solid ${col.color}`}}>
            <div style={{fontFamily:C,color:`${col.color}80`,fontSize:"clamp(.58rem,1.8vw,.65rem)",
              letterSpacing:".12em",marginBottom:8}}>{col.title}</div>
            {col.items.map((it,i)=>(
              <div key={i} style={{display:"flex",gap:6,marginBottom:4,alignItems:"flex-start"}}>
                <span style={{color:`${col.color}80`,marginTop:2,flexShrink:0}}>•</span>
                <span style={{fontFamily:F,fontSize:"clamp(.7rem,2.5vw,.8rem)",color:"#94a3b8",lineHeight:1.5}}>{it}</span>
              </div>
            ))}
          </div>
        ))}
      ...
      <div style={{padding:"clamp(.8rem,3vw,1rem)",border:`1px solid ${AM}40`,background:`${AM}08`,
        borderRadius:4,opacity:vis?1:0,transition:"opacity .6s .3s"}}>
        <div style={{display:"flex",gap:10,alignItems:"flex-start"}}>
          <div style={{fontSize:"1.2rem",flexShrink:0}}>📋</div>
          <div>
            <div style={{fontFamily:F,fontWeight:700,color:AM,fontSize:"clamp(.85rem,3vw,.95rem)",marginBottom:4}}>
              サブスク契約への移行が必須
            </div>
            <div style={{fontFamily:F,color:"#94a3b8",fontSize:"clamp(.72rem,2.5vw,.82rem)",lineHeight:1.6}}>
              安定的なサービス継続利用には、サブスクリプション契約への切り替えが必要です。
              移行後はサポート・保守の対象となり、安定運用を保証します。
            </div>
          </div>
        </div>
      </div>
    ...
    <Shell>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
        {[
          ...
          <div key={ci} style={{
            padding:"clamp(.8rem,3vw,1rem)",border:`1px solid ${col.color}25`,
            background:`${col.color}07`,borderRadius:4,
            opacity:vis?1:0,transform:vis?"translateY(0)":"translateY(16px)",
            transition:`all .5s ${ci*0.15}s`,
          }}>
            <div style={{fontSize:"1.5rem",marginBottom:4}}>{col.icon}</div>
            <div style={{fontFamily:C,color:`${col.color}70`,fontSize:"clamp(.55rem,1.8vw,.62rem)",
              letterSpacing:".15em",marginBottom:6}}>{col.label}</div>
            <div style={{fontFamily:F,fontWeight:700,color:"#e2e8f0",fontSize:"clamp(.82rem,3vw,.9rem)",
              marginBottom:8}}>{col.title}</div>
            {col.items.map((it,i)=>(
              <div key={i} style={{display:"flex",gap:5,marginBottom:3,alignItems:"center"}}>
                <span style={{color:`${col.color}70`,fontSize:".7rem"}}>→</span>
                <span style={{fontFamily:F,fontSize:"clamp(.68rem,2.3vw,.78rem)",color:"#94a3b8"}}>{it}</span>
              </div>
            ))}
          </div>
        ))}
      ...
      <div style={{padding:"clamp(.7rem,2.5vw,.9rem)",border:`1px solid ${GN}30`,background:`${GN}08`,
        borderRadius:4,opacity:vis?1:0,transition:"opacity .6s .4s"}}>
        <div style={{fontFamily:F,fontWeight:700,color:GN,fontSize:"clamp(.8rem,2.8vw,.88rem)",marginBottom:3}}>
          🎯 目指すゴール
        </div>
        <div style={{fontFamily:F,color:"#94a3b8",fontSize:"clamp(.7rem,2.3vw,.8rem)",lineHeight:1.6}}>
          「選ばれ続ける会員基盤」の構築
          ― 会員離脱防止 × 運用負担削減 × 単一プラットフォームへの機能統合
        </div>
      </div>
    </Shell>
  );
...
/* S4: PLATFORM - Three.js showcase */
function S4_Platform(){
  return (
    <Shell>
      <SLabel>PLATFORM / POINTQUICの全体像</SLabel>
      <h2 style={{fontFamily:F,fontSize:"clamp(1.2rem,4vw,1.8rem)",fontWeight:800,color:"#f1f5f9",
        lineHeight:1.3,margin:"0 0 .6rem"}}>
        取引データを<span style={{color:AM}}>自動でCRMに変換</span>する<br/>プラットフォーム
      </h2>
      <p style={{fontFamily:F,color:"#64748b",fontSize:"clamp(.72rem,2.5vw,.82rem)",
        lineHeight:1.7,margin:"0 0 .8rem"}}>
        POS・EC・決済から取引データを自動取得し、LINE/LIFF会員証に顧客を統合。
        セグメント・ランク・自動配信などのCRM施策を現場オペレーションを増やさず実行できます。
      </p>
      <CRMNetwork3D/>
      <div style={{marginTop:10,display:"flex",flexWrap:"wrap",gap:6}}>
        {["タブレット運用基本不要","POS自動連携","EC自動連携","会員証はLIFFアプリ","NFC対応"].map(t=>(
          <span key={t} style={{fontFamily:C,fontSize:"clamp(.55rem,1.8vw,.65rem)",
            padding:"3px 8px",border:`1px solid ${BL}30`,color:`${BL}70`,borderRadius:2}}>{t}</span>
        ))}
      </div>
    </Shell>
  );
...
function S5_Benefits(){
  ...
  return (
    <Shell>
      ...
      <h2 style={{fontFamily:F,fontSize:"clamp(1.2rem,4vw,1.8rem)",fontWeight:800,color:"#f1f5f9",
        lineHeight:1.2,margin:"0 0 clamp(.8rem,3vw,1.4rem)"}}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:8}}>
        {items.map((it,i)=>(
          <div key={i} style={{
            ...
            <div style={{flex:1}}>
              <div style={{fontFamily:C,fontSize:"clamp(.52rem,1.7vw,.6rem)",color:`${it.color}70`,
                letterSpacing:".1em",marginBottom:2}}>{it.badge}</div>
              <div style={{fontFamily:F,fontWeight:700,color:"#e2e8f0",fontSize:"clamp(.82rem,3vw,.92rem)",
                marginBottom:5}}>{it.title}</div>
              <div style={{display:"flex",flexWrap:"wrap",gap:"3px 10px"}}>
                {it.points.map(p=>(
                  <span key={p} style={{fontFamily:F,fontSize:"clamp(.65rem,2.2vw,.74rem)",color:"#64748b",
...
function S6_HowItWorks(){
  ...
  return (
    <Shell>
      ...
      <h2 style={{fontFamily:F,fontSize:"clamp(1.1rem,3.8vw,1.7rem)",fontWeight:800,color:"#f1f5f9",
        lineHeight:1.3,margin:"0 0 clamp(.7rem,2.5vw,1.2rem)"}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:14,
        ...
        {steps.map((st,i)=>(
          <div key={i} style={{display:"flex",alignItems:"center",flex:1,gap:6}}>
            <div style={{flex:1,padding:"clamp(.5rem,2vw,.8rem)",border:`1px solid ${BL}30`,
              background:`${BL}08`,borderRadius:4,textAlign:"center"}}>
              <div style={{fontFamily:C,color:`${BL}60`,fontSize:"clamp(.5rem,1.7vw,.6rem)",
                marginBottom:3}}>{st.who}</div>
...
function S7_Pricing(){
  ...
  return (
    <Shell>
      ...
      {/* Alert */}
      <div style={{padding:"clamp(.6rem,2vw,.8rem)",border:`1px solid ${RD}40`,background:`${RD}08`,
        borderRadius:4,marginBottom:12,display:"flex",gap:8,alignItems:"center",
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:12}}>
        {[
          {label:"単体機能",price:"15",unit:"万円",sub:"（税別）",highlight:false,
            features:["LINE会員証","専用アプリ","ICタッチ","POS/EC連携"],tag:null},
          {label:"パッケージ",price:"20",unit:"万円",sub:"（税別）",highlight:true,
            features:["複数機能を統合","5万で全機能追加","トータルコスト削減","統合的な運用が可能"],tag:"おすすめ"},
          {label:"月額費用",price:"6,000",unit:"円〜",sub:"（税別）",highlight:false,
            features:["会員数に応じた従量課金","スモールスタート可能","〜1,000人：月6,000円"],tag:null},
        ].map((p,i)=>(
          <div key={i} style={{
            padding:"clamp(.7rem,2.5vw,1rem)",borderRadius:4,position:"relative",
            ...
            {p.tag&&<div style={{position:"absolute",top:-10,left:"50%",transform:"translateX(-50%)",
              background:AM,color:"#000",fontFamily:F,fontWeight:700,
              fontSize:"clamp(.55rem,1.8vw,.65rem)",padding:"2px 8px",borderRadius:2}}>{p.tag}</div>}
            <div style={{fontFamily:C,color:`${p.highlight?AM:BL}80`,fontSize:"clamp(.52rem,1.7vw,.6rem)",
              letterSpacing:".1em",marginBottom:4}}>初期費用</div>
            <div style={{fontFamily:F,fontWeight:800,color:p.highlight?AM:"#e2e8f0",
              fontSize:"clamp(1.4rem,4.5vw,1.8rem)",lineHeight:1}}>
              {p.price}<span style={{fontSize:"clamp(.65rem,2vw,.75rem)",fontWeight:400}}>{p.unit}</span>
            </div>
            <div style={{fontFamily:F,color:"#475569",fontSize:"clamp(.6rem,2vw,.68rem)",
              marginBottom:8}}>{p.sub}</div>
            {p.features.map(f=>(
              <div key={f} style={{display:"flex",gap:4,marginBottom:2,alignItems:"flex-start"}}>
                <span style={{color:`${p.highlight?AM:BL}60`,fontSize:".65rem",marginTop:2}}>✓</span>
                <span style={{fontFamily:F,fontSize:"clamp(.62rem,2vw,.72rem)",color:"#64748b",lineHeight:1.4}}>{f}</span>
              </div>
            ))}
          </div>
        ))}
      ...
      <div style={{padding:"clamp(.7rem,2.5vw,.9rem)",border:`1px solid ${GN}30`,background:`${GN}08`,
        borderRadius:4,opacity:vis?1:0,transition:"opacity .5s .4s"}}>
...
function S8_Cases(){
  ...
  useEffect(()=>{setTimeout(()=>setVis(true),100);},[]);
  const voices=[
    {type:"会員様（エンドユーザー）の声",badge:"利便性向上",color:BL,
      quote:"お財布から会員証を探す手間がなく、スマホでサッと表示可能"},
    {type:"店舗様（オーナー）の声",badge:"集客効果",color:AM,
      quote:"クーポン配信の自動化により、お客様の来店頻度が向上"},
  ];
  const industries=[
  ...
  return (
    <Shell>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:14}}>
        {voices.map((v,i)=>(
          <div key={i} style={{
            padding:"clamp(.8rem,3vw,1.1rem)",border:`1px solid ${v.color}25`,
            background:`${v.color}07`,borderRadius:4,
            opacity:vis?1:0,transform:vis?"translateX(0)":"translateX(-16px)",
            transition:`all .5s ${i*.12}s`,
          }}>
            <div style={{display:"flex",gap:6,alignItems:"center",marginBottom:6}}>
              <div style={{fontFamily:F,fontSize:"clamp(.68rem,2.3vw,.76rem)",color:"#64748b"}}>{v.type}</div>
              <span style={{fontFamily:C,fontSize:"clamp(.5rem,1.6vw,.58rem)",
                padding:"2px 6px",background:`${v.color}20`,color:v.color,borderRadius:2}}>{v.badge}</span>
            </div>
            <div style={{fontFamily:C,color:v.color,fontSize:"1.2rem",marginBottom:4}}>"</div>
            <div style={{fontFamily:F,color:"#cbd5e1",fontSize:"clamp(.8rem,2.8vw,.92rem)",
              lineHeight:1.6,fontStyle:"italic"}}>{v.quote}</div>
          </div>
        ))}
      </div>

      <div style={{padding:"clamp(.8rem,3vw,1rem)",border:`1px solid rgba(14,165,233,.15)`,
        background:"rgba(14,165,233,.04)",borderRadius:4,marginBottom:10,
        opacity:vis?1:0,transition:"opacity .5s .3s"}}>
        <div style={{fontFamily:C,fontSize:"clamp(.55rem,1.8vw,.62rem)",color:`${BL}60`,
          letterSpacing:".12em",marginBottom:8}}>移行実績のある業界例</div>
        <div style={{display:"flex",gap:8}}>
          {industries.map(ind=>(
            <div key={ind.label} style={{flex:1,padding:"clamp(.5rem,2vw,.7rem)",
      ...
      <div style={{padding:"clamp(.6rem,2vw,.8rem)",border:`1px solid ${GN}25`,background:`${GN}06`,borderRadius:4,
        opacity:vis?1:0,transition:"opacity .5s .5s"}}>
        <div style={{fontFamily:C,fontSize:"clamp(.52rem,1.7vw,.6rem)",color:`${GN}70`,
          letterSpacing:".1em",marginBottom:6}}>主な導入企業</div>
        <div style={{display:"flex",flexWrap:"wrap",gap:"4px 10px"}}>
          {enterprises.map(e=>(
            <span key={e} style={{fontFamily:F,fontSize:"clamp(.62rem,2vw,.72rem)",color:"#64748b"}}>{e}</span>
          ))}
  ...
  return (
    <Shell>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:6}}>
        {qas.map((qa,i)=>(
          <div key={i} style={{border:`1px solid rgba(14,165,233,${open===i?.3:.15})`,
            borderRadius:4,overflow:"hidden",
            background:`rgba(14,165,233,${open===i?.06:.03})`,
            transition:"all .2s"}}>
            <button onClick={()=>{setOpen(open===i?null:i);sfxClick();}} style={{
              width:"100%",textAlign:"left",background:"none",border:"none",cursor:"pointer",
              padding:"clamp(.7rem,2.5vw,.9rem)",display:"flex",justifyContent:"space-between",
              alignItems:"center",gap:8}}>
              <span style={{fontFamily:F,fontWeight:600,color:"#e2e8f0",
                fontSize:"clamp(.78rem,2.8vw,.88rem)",lineHeight:1.4}}>Q. {qa.q}</span>
            ...
            {open===i&&(
              <div style={{padding:"0 clamp(.7rem,2.5vw,.9rem) clamp(.7rem,2.5vw,.9rem)",
                fontFamily:F,color:"#94a3b8",fontSize:"clamp(.72rem,2.5vw,.82rem)",lineHeight:1.7,
  ...
  return (
    <Shell>
      <div style={{textAlign:"center",display:"flex",flexDirection:"column",alignItems:"center",
        gap:"clamp(1.2rem,4vw,2rem)"}}>
        <div style={{opacity:ph>=1?1:0,transition:"opacity .8s",fontFamily:C,
          color:`rgba(16,185,129,.5)`,fontSize:"clamp(.65rem,2.3vw,.78rem)",letterSpacing:".25em"}}>
          SESSION COMPLETE
        </div>
        <div style={{opacity:ph>=2?1:0,transition:"opacity 1s",
          transform:ph>=2?"translateY(0)":"translateY(20px)"}}>
          <div style={{fontFamily:F,fontWeight:900,fontSize:"clamp(1.5rem,6vw,3.2rem)",
            color:"#f1f5f9",lineHeight:1.15,letterSpacing:"-.01em"}}>
            今日の商談で、<br/>
            <Glitch style={{color:BL}}>何か気付き</Glitch>は<br/>ありましたか？
          </div>
        </div>
        <div style={{opacity:ph>=3?1:0,transition:"opacity .8s",maxWidth:420}}>
          <div style={{fontFamily:F,color:"#64748b",fontSize:"clamp(.8rem,2.8vw,.95rem)",
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8,alignItems:"center"}}>
            {[
              ...
              <div key={l.label} style={{fontFamily:C,padding:"6px 16px",
                border:`1px solid rgba(14,165,233,.3)`,color:BL,
                fontSize:"clamp(.68rem,2.3vw,.78rem)",borderRadius:2,letterSpacing:".05em"}}>
                {l.icon} {l.label} <span style={{color:"rgba(14,165,233,.4)"}}>— {l.sub}</span>
              </div>
            ))}
          </div>
        </div>
...
export default function App(){
  ...
  return (
    <div style={{width:"100vw",height:"100vh",background:BG,color:"#f1f5f9",
      overflow:"hidden",position:"fixed",inset:0,fontFamily:F}}>
      <style>{`
      ...
      <div style={{position:"fixed",top:0,left:0,right:0,zIndex:10,
        ...
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          <div style={{width:8,height:8,borderRadius:"50%",background:BL,
            boxShadow:`0 0 8px ${BL}`}}/>
          <span style={{fontFamily:C,fontSize:"clamp(.55rem,1.8vw,.65rem)",
            color:`rgba(14,165,233,.6)`,letterSpacing:".15em"}}>POINTQUIC</span>
        </div>
      ...
      {/* Slide area */}
      <div {...swipe} style={{position:"fixed",inset:0,paddingTop:"clamp(2.5rem,8vw,3.2rem)",
        paddingBottom:"clamp(3.5rem,12vw,4.5rem)",
