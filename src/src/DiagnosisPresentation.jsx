      ...
      <div style={{display:"flex",flexDirection:"column",gap:8}}>
        ...
        <div style={{display:"flex",gap:14,alignItems:"center",padding:".85rem 1rem",background:SF,border:`1px solid ${GN}30`,borderRadius:8,borderLeft:`3px solid ${GN}`,boxShadow:"0 1px 4px rgba(0,0,0,.06)"}}>
          ...
          <div>
            ...
            <div style={{fontFamily:F,color:TX2,fontSize:".82rem",lineHeight:1.6}}>未来の売上を今日受け取る仕組み。チャージされた金額はキャンセルがない限り確実に売上になります。</div>
          </div>
        </div>
    ...
    <Shell>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:8}}>
        ...
        <div style={{display:"flex",gap:14,alignItems:"center",padding:".85rem 1rem",background:SF,border:`1px solid ${GNL}25`,borderRadius:8,borderLeft:`3px solid ${GNL}`,boxShadow:"0 1px 4px rgba(0,0,0,.06)"}}>
          ...
          <div>
            <div style={{fontFamily:F,fontWeight:700,color:TX,fontSize:"1rem",marginBottom:2}}>顧客属性に合わせた配信でリピート率が上がる</div>
            <div style={{fontFamily:F,color:TX2,fontSize:".82rem",lineHeight:1.55}}>一律配信ではなく、来店頻度・ポイント残高・最終来店日でセグメントを分けて最適なメッセージを届けます。</div>
          </div>
        </div>
        ...
        <Accordion icon="🎯" title="セグメント自動仕分け" sub="3種類の顧客に自動で最適な配信" color={GNL} defaultOpen={false}>
          {[
            {label:"VIP顧客",count:"上位20%",color:AM,  rate:"限定オファー",msg:"特別感のある限定オファー"},
            {label:"休眠顧客",count:"90日+未来店",color:RD,rate:"呼び戻し",msg:"「お久しぶり」クーポン"},
            {label:"新規顧客",count:"初回〜3回",color:GNL,rate:"リピーター化",msg:"次回来店を促すリピーター化"},
          ].map((s,i)=>(
        ...
        <Accordion icon="📱" title="配信画面イメージ" sub="管理画面からワンクリック" color={BL}>
          <PhoneMockup color={GNL} title="LINE SEGMENT">
            <div style={{padding:"2px 0"}}>
              {[
                {l:"VIP",n:"上位層",c:AM,r:"限定"},
                {l:"休眠",n:"未来店",c:RD,r:"呼戻"},
                {l:"新規",n:"初回〜",c:GNL,r:"定着"},
              ].map(s=>(
    ...
    <Shell>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:8}}>
        ...
        <div style={{display:"flex",gap:14,alignItems:"center",padding:".85rem 1rem",background:SF,border:`1px solid ${AM}25`,borderRadius:8,borderLeft:`3px solid ${AM}`,boxShadow:"0 1px 4px rgba(0,0,0,.06)"}}>
          <div style={{fontFamily:C,fontSize:"clamp(1.5rem,4.5vw,2rem)",color:AM,fontWeight:700,lineHeight:1,flexShrink:0}}>-30<span style={{fontSize:"55%"}}>秒</span></div>
          <div>
            <div style={{fontFamily:F,fontWeight:700,color:TX,fontSize:"1rem",marginBottom:2}}>処理時間 30秒短縮</div>
            <div style={{fontFamily:F,color:TX2,fontSize:".82rem",lineHeight:1.55}}>QRスキャンで受付が完結。手入力や手渡し不要でピーク時の行列を大幅に解消できます。</div>
          </div>
        </div>
        ...
        <Accordion icon="⏱" title="処理時間の比較" sub="1件あたりの所要時間" color={AM}>
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            {[
              {label:"従来（カード＋手入力）",time:100,max:100,color:RD},
              {label:"POINTQUIC（QRスキャン）",time:55,max:100,color:AM},
            ].map((row,i)=>(
              <div key={i}>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:4,alignItems:"center"}}>
                  <span style={{fontFamily:F,fontSize:".82rem",color:TX2}}>{row.label}</span>
                  <span style={{fontFamily:C,fontSize:".85rem",color:row.color,fontWeight:700}}>{row.label.includes("POINTQUIC")?"約30秒短縮":"（比較基準）"}</span>
                </div>
